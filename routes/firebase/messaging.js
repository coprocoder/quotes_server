...
router.post('/test', async (req, res) => {
  ...
  fb_log_file.write(util.format(log_obj) + '\n');  
...
router.post('/send', async (req, res) => {
    ...
    .then((get_users_results) => {
      ...
      db
        ...
        .then((get_secure_results) => {
          ...
          token_list = Object.values(get_secure_results[0].fb_token)
          // console.log('register_token token_list', token_list)
        })
        .catch((err) => {
          next(err);
        })
  ...
  var registrationTokens = token_list;
