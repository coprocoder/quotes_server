...
router.post("/register_token", (req, res, next) => {
  ...
  */
  ...
  // Ищем юзера с email из jwt_token
  db.get(db.users_database, db.users_collection, filter, get_user_fields)
      ...
      db.get(db.secure_database, db.secure_collection, filter_secure, get_secure_fields)
        .then((get_secure_results) => {
          ...
          let update_secure_fields = {
            fb_token: Object.assign({}, get_result_field.fb_token, new_token),
          };
          ...
          db.update(db.secure_database, db.secure_collection, filter_secure, update_secure_fields)
            .then((results) => {
              if (!!results) {
                res.send({
...
router.post("/delete_token", (req, res, next) => {
  ...
  */
  ...
  db.get(db.users_database, db.users_collection, filter, get_user_fields).then((get_users_results) => {
    ...
    db.remove(db.secure_database, db.secure_collection, filter_secure, remove_secure_fields).then(
      (remove_secure_results) => {
        res.send({
          message: "Данные обновлены",
        });
      }
